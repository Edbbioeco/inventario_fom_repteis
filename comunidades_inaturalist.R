...
inaturalist_sf_fom <- inaturalist_sf_fom |>
  dplyr::mutate(
    scientific_name = scientific_name |> stringr::str_squish(),
    ...
    scientific_name = scientific_name |> stringr::str_replace("Sibynomorphus", "Dipsas")
  ) |>
  dplyr::mutate(
    scientific_name = dplyr::case_match(
      scientific_name,
    ...
    taxon_family_name = dplyr::case_when(
      scientific_name |> stringr::str_detect("Urostrophus") ~ "Iguanidae",
      scientific_name |> stringr::str_detect("Enyalius") ~ "Leiosauridae",
      scientific_name |> stringr::str_detect("Liotyphlops") ~ "Anomalepididae",
      scientific_name |> stringr::str_detect("Notomabuya") ~ "Scincidae",
      scientific_name |> stringr::str_detect("Ophiodes") ~ "Diploglossidae",
      scientific_name |> stringr::str_detect("Palusophis") ~ "Colubridae",
      scientific_name |> stringr::str_detect("Podocnemis") ~ "Podocnemididae",
      scientific_name |> stringr::str_detect("Apostolepis|Atractus|Boiruna|Clelia|Dibernardia|Dipsas|Dryophylax|Echinanthera|Erythrolamprus|Gomesophis|Oxyrhopus|Helicops|Imantodes|Mesotes|Paraphimophis|Phalotris|Philodryas|Pseudoboa|Ptychophis|Rhachidelus|Siphlophis|Taeniophallus|Thamnodynastes|Tomodon|Tropidodryas|Cercophis|Xenodon|Lygophis|Adelphostigma|Pseudablabes") ~ "Dipsadidae",
      taxon_family_name == "Varanidae" ~ "Teiidae",
      taxon_family_name |> stringr::str_detect("Xenodon") ~ "Dipsadidae",
      taxon_family_name |> stringr::str_detect("inae") ~ taxon_family_name |> stringr::str_replace("inae", "idae"),
      taxon_family_name == "Varanidae" ~ "Teiidae",
