...
herpetohelp_sf_fom <- herpetohelp_sf_fom |>
  dplyr::mutate(
    Espécie = Espécie |> stringr::str_squish(),
    ...
    Espécie = Espécie |> stringr::str_replace("Sibynomorphus", "Dipsas")
  ) |>
  dplyr::mutate(
    Espécie = dplyr::case_match(
      Espécie,
    ...
    Família = dplyr::case_when(
      Espécie |> stringr::str_detect("Urostrophus") ~ "Iguanidae",
      Espécie |> stringr::str_detect("Enyalius") ~ "Leiosauridae",
      Espécie |> stringr::str_detect("Liotyphlops") ~ "Anomalepididae",
      Espécie |> stringr::str_detect("Notomabuya") ~ "Scincidae",
      Espécie |> stringr::str_detect("Ophiodes") ~ "Diploglossidae",
      Espécie |> stringr::str_detect("Palusophis") ~ "Colubridae",
      Espécie |> stringr::str_detect("Podocnemis") ~ "Podocnemididae",
      Espécie |> stringr::str_detect("Apostolepis|Atractus|Boiruna|Clelia|Dibernardia|Dipsas|Dryophylax|Echinanthera|Erythrolamprus|Gomesophis|Oxyrhopus|Helicops|Imantodes|Mesotes|Paraphimophis|Phalotris|Philodryas|Pseudoboa|Ptychophis|Rhachidelus|Siphlophis|Taeniophallus|Thamnodynastes|Tomodon|Tropidodryas|Cercophis|Xenodon|Lygophis|Adelphostigma|Pseudablabes") ~ "Dipsadidae",
      Família == "Varanidae" ~ "Teiidae",
      Família |> stringr::str_detect("Xenodon") ~ "Dipsadidae",
      Família |> stringr::str_detect("inae") ~ Família |> stringr::str_replace("inae", "idae"),
      Família == "Varanidae" ~ "Teiidae",
      .default = Família
    ),
