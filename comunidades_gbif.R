...
gbif_sf_fom <- gbif_sf_fom |>
  ...
  dplyr::mutate(
    species = dplyr::case_match(
      ...
      c("Varanus salvator", "Tupinambis merianae", "Tupinambis teguixin", "Tupinambis teguixim", "Salvator marianae", "Salvator rufescens") ~ "Salvator merianae",
  ...
  dplyr::filter(
    ...
    !(species |> stringr::str_trim() |> stringr::str_count("\\S+") == 1))
